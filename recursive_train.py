...
def main():
    ...
    if args.eval_only:
        ...
        print(f"[EVAL ONLY] PSNR: {mean_psnr:.4f}, SSIM: {mean_ssim:.4f}")
        logging.info(f"[EVAL ONLY] PSNR: {mean_psnr:.4f}, SSIM: {mean_ssim:.4f}")
        writer.add_scalar("Val/PSNR", mean_psnr, 0)
        writer.add_scalar("Val/SSIM", mean_ssim, 0)
        return
    ...
    for epoch in range(args.epochs):
        ...
        print(f"[VAL@Epoch {epoch}] PSNR: {mean_psnr:.4f}, SSIM: {mean_ssim:.4f}")
        logging.info(f"[VAL@Epoch {epoch}] PSNR: {mean_psnr:.4f}, SSIM: {mean_ssim:.4f}")
